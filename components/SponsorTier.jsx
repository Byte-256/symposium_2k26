...
export default function SponsorTier({ tier, index }) {
  ...
  return (
    ...
    >
      {featured && (
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 bg-gradient-to-r from-cyan-500 to-purple-500 rounded-full text-xs font-medium text-white">
          Most Popular
        </div>
      )}
      ...
      <div className={`h-full p-8 rounded-2xl border transition-all duration-300 ${
        ...
        <div className="text-center mb-8">
          <div className={`inline-flex px-4 py-1.5 rounded-full text-sm font-medium mb-4 ${color}`}>
            {name}
          </div>
          ...
          {benefits.map((benefit, i) => (
            <li key={i} className="flex items-start gap-3">
              <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${
                ...
                <Check size={12} className={featured ? 'text-cyan-400' : 'text-slate-400'} />
              </div>
